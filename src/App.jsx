...
const Ph=({h=200,l="Image Placeholder"})=><div style={{width:"100%",height:h,background:"linear-gradient(135deg,#111,#1a1a1a)",borderRadius:10,border:"1px dashed #333",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:8}}><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg><span style={{fontSize:11,color:"#444"}}>{l}</span></div>;
const SV=[
{id:"w",ic:"w",t:"Web Design & Development",sh:"Custom, responsive websites that convert visitors into customers.",fl:"Your website is the foundation of your online presence. We craft pixel-perfect, responsive websites optimized for speed, SEO, and conversions.",ft:["Custom responsive design","WordPress & CMS development","E-commerce solutions","Landing page optimization","Website maintenance","SEO-optimized architecture"],pr:["Discovery call","Wireframe & design","Development & testing","Launch & support"],img:webDevImg},
{id:"m",ic:"m",t:"Mobile App Development",sh:"Native and cross-platform apps with intuitive design.",fl:"We turn your app ideas into reality with native iOS/Android or cross-platform solutions that users love.",ft:["iOS & Android development","Cross-platform solutions","UI/UX mobile design","App store optimization","Push notifications","Ongoing maintenance"],pr:["Strategy & planning","UI/UX prototyping","Agile development","Testing & launch"],img:mobileAppImg},
{id:"s",ic:"s",t:"Search Engine Optimization",sh:"Data-driven SEO strategies that boost rankings.",fl:"Our SEO strategies are built on real data, competitor analysis, and proven techniques that move the needle on rankings.",ft:["Technical SEO audits","Keyword research","On-page optimization","Link building","Local SEO Houston","Monthly analytics"],pr:["Comprehensive audit","Strategy development","Implementation","Monitor & refine"],img:seoImg},
];
const PR=[
{n:"Takween AI",c:"Web Design & AI",y:"2025",img:PJ.t,d:"Built a cutting-edge AI services platform with modern design."},
{n:"MySmartFit Chair",c:"E-Commerce",y:"2023",img:PJ.m,d:"Developed a full e-commerce solution for innovative furniture."},
{n:"Slayers Club",c:"Branding & Social",y:"2022",img:PJ.s,d:"Created branding and social media strategy for gaming community."},
{n:"SimpleTouch POS",c:"Web Application",y:"2021",img:PJ.p,d:"Designed a point-of-sale web app with companion mobile app."}
];
const TE=[
{q:"NetTracePro understands how to communicate with our customers. Their approach transformed our entire digital presence.",a:"MySmartFit Chair",r:"E-Commerce Client",bg:"linear-gradient(135deg,#E8A020,#F0C060)"},
{q:"We trusted them and they delivered. Two years in, our social channels are growing faster than ever.",a:"Slayers Club Team",r:"Gaming Community",bg:"linear-gradient(135deg,#4CAF50,#2E7D32)"},
{q:"We rely on NetTracePro for their professionalism and dedication. They consistently exceed expectations.",a:"Neuma Home",r:"Home Services",bg:"linear-gradient(135deg,#42A5F5,#1565C0)"}
];
const Nav=({go,sc})=>{const[dd,sdd]=useState(false);return<nav className={`nv ${sc?"sc":""}`}><a href="#" onClick={e=>{e.preventDefault();go("home")}} className="nl"><img src={LN} alt="NTP" style={{height:36}}/></a><div className="nk">
<div className="ndd" onMouseEnter={()=>sdd(true)} onMouseLeave={()=>sdd(false)}><a href="#" onClick={e=>{e.preventDefault();go("services")}}>Services</a>{dd&&<div className="ndm">{SV.map(s=><a key={s.id} href="#" onClick={e=>{e.preventDefault();go("sv-"+s.id);sdd(false)}}>{s.t}</a>)}</div>}</div>
...
const Ft=({go})=><footer className="ft"><div className="fw"><div className="fg"><div className="fb"><a href="#" onClick={e=>{e.preventDefault();go("home")}} className="nl"><img src={LF} alt="NTP" style={{height:50}}/></a><p>A full-service digital agency empowering businesses with effective digital strategies.</p></div>
<div className="fc"><h4>Company</h4><a href="#" onClick={e=>{e.preventDefault();go("about")}}>About Us</a><a href="#" onClick={e=>{e.preventDefault();go("testimonials")}}>Testimonials</a><a href="#" onClick={e=>{e.preventDefault();go("portfolio")}}>Portfolio</a></div>
<div className="fc"><h4>Services</h4>{SV.map(s=><a key={s.id} href="#" onClick={e=>{e.preventDefault();go("sv-"+s.id)}}>{s.t.split("&")[0].trim()}</a>)}</div>
<div className="fc"><h4>Connect</h4><a href="#" onClick={e=>{e.preventDefault();go("contact")}}>Contact Us</a><a href="tel:+17132699658">(713) 269-9658</a><a href="mailto:info@nettracepro.com">info@nettracepro.com</a></div></div>
<div className="fbt"><p>© 2025 NetTracePro. All rights reserved.</p></div></div></footer>;
...
<section className="sec"><F><div className="hdr"><div className="label">Our Work</div><h2 className="title">Latest Projects</h2></div></F>
<div className="pg">{PR.map((p,i)=><F key={i} d={i*.08}><div className="pc"><div className="piw"><img className="pim" src={p.img} alt={p.n}/><div className="pn">0{i+1}</div></div><div className="pbd"><div className="pct">{p.c}</div><h3>{p.n}</h3><div className="py">{p.y}</div></div></div></F>)}</div>
...
<section className="sec-w ts"><div className="fw"><F><div className="hdr" style={{textAlign:"center"}}><div className="label">Testimonials</div><h2 className="title">What Our Clients Say</h2></div>
<div className="tcard"><div className="tglow"/><div className="tcr tcr1"/><div className="tcr tcr2"/>
<div className="tin">{TE.map((t,i)=><div key={i} className={`ti ${at===i?"act":""}`}><div className="tst">{[...Array(5)].map((_,j)=><span key={j}><S/></span>)}</div><p className="tq">"{t.q}"</p><div className="tar"><div className="tav" style={{background:t.bg}}>{IC.u}</div><div><div className="ta">{t.a}</div><div style={{fontSize:11,color:"var(--td)"}}>Verified Client</div></div></div></div>)}</div>
<div className="tds">{TE.map((_,i)=><button key={i} className={`tdt ${at===i?"act":""}`} onClick={()=>sa(i)}/>)}</div></div></F></div></section>
<div className="cta"><F><h2 className="cta-h">Ready to Build Something <span className="acc">Great</span>?</h2><p className="cta-p">Let us turn your vision into a high-performing digital experience.</p><a href="#" onClick={e=>{e.preventDefault();go("contact")}} className="btn bg" style={{position:"relative"}}>Start Your Project <A/></a></F></div></>;};

/* === SERVICES OVERVIEW === */
const Services=({go})=><><PB l="Our Services" t="What We Offer" s="Full-service digital solutions designed to grow your business online."/>
<section className="sec">{SV.map((s,i)=><F key={s.id} d={i*.1}><div className="svr" style={{flexDirection:i%2===1?"row-reverse":"row"}}><div className="svr-img"><img src={s.img} alt={s.t} style={{width:'100%',height:400,objectFit:'contain',borderRadius:10,background:'#0d0d0d'}}/></div><div className="svr-txt"><div className="si" style={{marginBottom:16}}>{IC[s.ic]}</div><h2 className="title" style={{fontSize:26}}>{s.t}</h2><p style={{color:"var(--tm)",lineHeight:1.7,fontWeight:300,marginBottom:20}}>{s.fl}</p><div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:24}}>{s.ft.slice(0,4).map((f,j)=><span key={j} className="ftag">{IC.k} {f}</span>)}</div><a href="#" onClick={e=>{e.preventDefault();go("sv-"+s.id)}} className="btn bg">Learn More <A/></a></div></div>{i<SV.length-1&&<div style={{height:1,background:"var(--bd)",margin:"50px 0"}}/>}</F>)}</section>
<div className="cta"><F><h2 className="cta-h">Ready to Get Started?</h2><p className="cta-p">Let's discuss which services are right for your business.</p><a href="#" onClick={e=>{e.preventDefault();go("contact")}} className="btn bg" style={{position:"relative"}}>Contact Us <A/></a></F></div></>;

/* === SERVICE DETAIL === */
const SvcDetail=({id,go})=>{const sv=SV.find(x=>x.id===id)||SV[0];return<><PB l="Service Detail" t={sv.t} s={sv.sh}/>
...
<FX dir="r"><div style={{display:"flex",flexDirection:"column",gap:16}}><img src={sv.img} alt={sv.t} style={{width:'100%',height:350,objectFit:'contain',borderRadius:10,background:'#0d0d0d'}}/><div style={{background:"var(--sf)",border:"1px solid var(--bd)",borderRadius:12,padding:24}}><h3 style={{color:"var(--w)",fontSize:15,fontWeight:600,marginBottom:16}}>Our Process</h3>{sv.pr.map((p,i)=><div key={i} className="prs"><div className="prn">0{i+1}</div><div className="prt">{p}</div></div>)}</div><a href="#" onClick={e=>{e.preventDefault();go("contact")}} className="btn bg" style={{textAlign:"center",justifyContent:"center"}}>Start This Service <A/></a></div></FX></div></section></>};

/* === ABOUT === */
const About=({go})=><><PB l="About Us" t="Your Success Is Our Mission" s="We're not just building websites — we're building partnerships."/>
...
<p style={{color:"var(--tm)",lineHeight:1.8,fontWeight:300,marginBottom:18}}>We've felt the frustration of a website that doesn't convert and agencies that talk big but don't deliver. That's exactly why we do things differently.</p>
<p style={{color:"var(--tm)",lineHeight:1.8,fontWeight:300}}>When you work with us, you're getting a team that <strong style={{color:"var(--g)"}}>listens first, understands your goals, and builds something tailored to your needs.</strong> Every pixel, every strategy is crafted with YOUR customers in mind.</p></div></FX>
<FX dir="r"><div style={{display:"flex",flexDirection:"column",gap:14}}><img src={teamPhoto} alt="Team Photo" style={{width:'50%', height:280, objectFit:'contain', borderRadius:10, display:'block', margin:'0 auto'}}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>{[{n:"15+",l:"Happy Clients"},{n:"4+",l:"Years Strong"},{n:"100%",l:"Satisfaction"},{n:"24/7",l:"Support"}].map((s,i)=><div key={i} className="abst"><div className="stn" style={{fontSize:22}}>{s.n}</div><div style={{fontSize:10,color:"var(--td)",textTransform:"uppercase",letterSpacing:1}}>{s.l}</div></div>)}</div></div></FX></div></section>
<section className="sec" style={{paddingTop:0}}><F><div className="hdr" style={{textAlign:"center"}}><div className="label">Our Process</div><h2 className="title">How We Take Care of You</h2><p className="sub" style={{margin:"0 auto",textAlign:"center"}}>Every project follows our proven process — designed to keep you informed and confident.</p></div></F>
<div className="prg">{[{n:"01",t:"We Listen",d:"We start with a real conversation about your vision, customers, and goals.",i:"💬"},{n:"02",t:"We Plan",d:"Together we map out strategy, timelines, and deliverables. No surprises.",i:"📋"},{n:"03",t:"We Build",d:"Our team brings your vision to life with clean code and stunning design.",i:"⚡"},{n:"04",t:"We Launch",d:"Go-live is just the beginning. We provide ongoing support as you grow.",i:"🚀"}].map((s,i)=><F key={i} d={i*.1}><div className="prc"><div className="prc-n">{s.n}</div><div style={{fontSize:28,marginBottom:10}}>{s.i}</div><h3 style={{color:"var(--w)",fontSize:17,fontWeight:600,marginBottom:6}}>{s.t}</h3><p style={{color:"var(--tm)",fontSize:13,lineHeight:1.6,fontWeight:300}}>{s.d}</p></div></F>)}</div></section>
<div className="cta"><F><h2 className="cta-h">Let's Build <span className="acc">Together</span></h2><p className="cta-p">Your vision. Our expertise. A website that works for your business.</p><a href="#" onClick={e=>{e.preventDefault();go("contact")}} className="btn bg" style={{position:"relative"}}>Start the Conversation <A/></a></F></div></>;
...
/* === TESTIMONIALS === */
const Testimonials=({go})=><><PB l="Testimonials" t="Our Clients Love Us" s="Don't just take our word for it — hear from businesses we've helped grow."/>
...
<p style={{color:"var(--tm)",fontSize:14,lineHeight:1.7,fontWeight:300,fontStyle:"italic",marginBottom:16}}>"{t.q}"</p><Ph h={120} l="Project Screenshot"/></div></F>)}</div>
...
/* === CONTACT === */
const Contact=({go})=>{const[f,sf]=useState({n:"",e:"",p:"",s:"",m:""});return<><PB l="Contact Us" t="Let's Start Something Great" s="Tell us about your project — we'll respond within 24 hours."/>
<section className="sec"><div className="ctg"><FX dir="l"><div><h2 className="title" style={{fontSize:22}}>Get in Touch</h2><p style={{color:"var(--tm)",lineHeight:1.7,fontWeight:300,marginBottom:28}}>Whether you have a detailed brief or just a rough idea, we'd love to hear from you. No pressure — just a real conversation.</p>
...
<FX dir="r"><div className="ctfc"><div className="ctfg"/><h3 style={{color:"var(--w)",fontSize:18,fontWeight:600,marginBottom:18,position:"relative"}}>Send Us a Message</h3>
...
<textarea className="fi fta" placeholder="Tell us about your project..." value={f.m} onChange={e=>sf({...f,m:e.target.value})}/><button className="fbt" onClick={()=>{alert("Thank you! We'll be in touch within 24 hours.");sf({n:"",e:"",p:"",s:"",m:""})}}>Send Message <A/></button></div></div></FX></div></section></>};

/* === APP === */
export default function App(){const[pg,sp]=useState("home");const[sc,ss]=useState(false);
...
.ndd{position:relative;display:flex;align-items:center}.ndm{position:absolute;top:100%;left:-12px;background:var(--sf);border:1px solid var(--bd);border-radius:8px;padding:8px 0;min-width:240px;margin-top:8px;box-shadow:0 20px 40px rgba(0,0,0,.4);z-index:100}.ndm a{display:block;padding:10px 20px;font-size:13px;border:none}.ndm a:hover{color:var(--g);background:var(--gd)}
...
.ft{padding:50px 60px 24px;border-top:1px solid var(--bd)}.fg{display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:40px;margin-bottom:32px}.fb p{font-size:12px;line-height:1.7;color:var(--td);font-weight:300;margin-top:12px;max-width:260px}.fc h4{font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:var(--g);font-weight:600;margin-bottom:14px}.fc a{display:block;font-size:12px;color:var(--td);text-decoration:none;margin-bottom:8px;font-weight:300}.fc a:hover{color:var(--g)}.fbt2{border-top:1px solid var(--bd);padding-top:16px}.fbt2 p{font-size:11px;color:var(--td)}
